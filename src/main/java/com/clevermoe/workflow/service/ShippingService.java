...
public class ShippingService {
    ...
    public ShippingOption[] execute(JSONObject input) {
        String url = "http://localhost:8080/api/shippingoption/find";
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<String> request = new HttpEntity<>(input.toString(), headers);
        ResponseEntity<ShippingOption[]> response = restTemplate.postForEntity(url, request,
